class ExercisesController < ApplicationController
  ...
  def exercise3 
    ...
    @address = Address.joins(:orders).group(:id).order('COUNT(orders.id) DESC').first
    orders_count = @address.orders.size
  end
  ...
  def exercise4 
    ...
    @customer = Customer.joins(orders: { order_foods: :food }).select('customers.*, SUM(foods.price) AS total_spent').group('customers.id').order('total_spent DESC').first
    foods_price_sum = @customer.total_spent
  end
end
